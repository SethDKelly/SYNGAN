# Current authority routing

**GENERATED OKF PROJECTION — DO NOT HAND-EDIT.**

Cross-cutting repository authority and progression routes.

- [Current Repository Status](current-repository-status.md) - Current repository progression, authorization, and next-boundary status.
- [Documentation Governance](documentation-governance.md) - Canonical documentation authority, anti-drift, and progressive-disclosure rules.
- [Canonical Knowledge Ownership Map](canonical-knowledge-ownership-map.md) - Preferred current-owner families for repository knowledge.
- [Completed Phase 016 Hardening Authority](phase-016-hardening-authority.md) - Completed Phase 016 hardening authority and continuing repository guardrails.
- [OKF v0.2 Producer Profile](okf-producer-profile.md) - Canonical producer, generated-projection, and conformance contract for OKF v0.2.
- [Stable Reference Contract](stable-reference-contract.md) - Canonical stable-reference, deterministic-resolution, and drift-control contract.
- [Agent Authority, Human-Directed Scope, Change Classes, Security & Trust Boundaries](agent-development-policy.md) - Canonical tool-neutral agent authority, scope, action-class, security, and trust contract.
- [Agent Context, Portable Workflows & Tool Adapter Contract](agent-context-workflows.md) - Canonical bounded-context, portable-skill, and provider-adapter contract.
- [Agentic Conformance, Negative Controls, Drift Detection & CI](agentic-conformance.md) - Canonical deterministic agentic conformance, negative-control, drift, and CI contract.

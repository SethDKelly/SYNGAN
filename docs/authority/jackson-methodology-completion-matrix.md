...
| B4 | Explicit familiarity/reuse comparison | 008-F compared all eleven individually; Phase 010 aligned owner-qualified vocabulary; 011-C replays the final composed application family against SDV, Spark ML, MLflow, Great Expectations, OpenLineage and retained PyTorch analogues, retaining 11/11 canonical names with qualified aliases only | **CURRENTLY CLOSED** | 008-F/H; 010-D/H; 011-C |
...
| C1 | Concept name and distinct purpose | 008-B/F; 011-B revalidates all eleven purposes in full composition; 011-C retains all names after external familiarity comparison | **CURRENTLY CLOSED** | 008-B/F/H; 011-B/C |
...
| D1 | Jackson application inclusion-dependence graph | 009-A/B establish graph/SCCs; Phase 010 creates no universal edge; 011-B/C find no specificity/familiarity-driven edge change | **CURRENTLY CLOSED** | 009-A/B/H; 011-B/C |
| D2 | Meaningful valid concept subsets/application family | 009-C family + side conditions; 010-F/G/H preserve valid subsets; 011-B/C use reduced-family survival/reuse as quality evidence | **CURRENTLY CLOSED** | 009-C/H; 010-F/G/H; 011-B/C |
...
| E1 | Explicit concept synchronizations | 13 active from 15 historical IDs; no hidden new synchronization required by Phase 010 or 011-B/C | **CURRENTLY CLOSED** | 009-E/F/G/H; 010-H; 011-B/C |
| E2 | Singular state ownership across synchronizations | 009-F ownership survives action/inspection/surface/family/parity mapping; 011-B/C find no specificity/familiarity-driven ownership collapse | **CURRENTLY CLOSED** | 009-F/G/H; 010-H; 011-B/C |
| E3 | Composition burden/economy and hidden-coordinator avoidance | 010-F rejects generic full-suite workflow; 010-G/H reject generic Recovery/Degraded/Status authorities; 011-B confirms no umbrella owner and 011-C rejects ecosystem aggregate nouns as canonical substitutes | **CURRENTLY CLOSED** | 009-F/G/H; 010-F/G/H; 011-B/C |
...
| F3 | Linguistic mapping/vocabulary alignment | 010-D aligns owner-qualified vocabulary, typed status, disclosure/history and ecosystem aliases; 011-C strengthens one-way compatibility aliases without changing owner/lifecycle semantics or requiring mapping correction | **CURRENTLY CLOSED** | 010-D/H; 011-C revalidation |
...
| G2 | Familiarity across final composed set | 011-C compares conceptual jobs against current SDV/Spark ML/MLflow/Great Expectations/OpenLineage and retained PyTorch vocabulary; all 11 canonical names retained, application-family reuse passes, R010-02 is NO DEFECT with strengthened compatibility guidance, no MAT-2/MAT-3 finding | **CURRENTLY CLOSED** | 011-C |
...
| G7 | Explicit residual conceptual misfit register | 011-A establishes mandatory finding/disposition vocabulary; 011-B contributes R010-01 NO DEFECT plus bounded Strategy/Provenance watch points; 011-C contributes R010-02 NO DEFECT plus bounded Data Meaning/Learned State/Evaluation Criterion vocabulary watch points; complete register remains | **PARTIAL** | 011-I/J; 012 confirmation |
...
[Familiarity, Reuse, Vocabulary & External-Model Comparison Audit](composed-familiarity-reuse-vocabulary-external-model-audit.md) establishes:
...
- familiar synthetic-data/ML objects frequently bundle concerns that SYNGAN intentionally separates;
- `fit`/`train`, `model`, `sample`, `validation result` and `lineage` are useful only as qualified/contextual compatibility terms;
- generic `synthesizer`, `metadata`, `run`, `job`, `artifact`, `metric`, `validation`, `result` and `lineage` aliases would erase current ownership or lifecycle distinctions;
- `Data Meaning`, `Learned State` and `Evaluation Criterion` have bounded first-use familiarity cost, but their familiar replacements introduce larger semantic distortion;
- the vocabulary remains stable across authority-only, direct Generation, learned-state-assisted Generation, evaluation-focused, Execution-bearing and Provenance-bearing family members;
- Phase 010-D linguistic mapping remains current and does not reopen.
...
```text
011-D -> 011-E -> 011-F -> 011-G -> 011-H -> 011-I -> 011-J
  ↓
...
Phase 011 may expose a genuine upstream misfit, but representation or implementation convenience is not evidence to redefine a concept. External ecosystem vocabulary may improve recognition but remains compatibility evidence unless its semantic job matches current authority exactly. Reopen only the smallest affected authority when a real misfit is established.
